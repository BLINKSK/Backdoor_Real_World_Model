...
logger = logging.getLogger('dev.aaptlib')
...
class ApkInvalidError(Exception):
...
class ApkInfo():
    ...
    def getDumpBadging(self):
        ...
        for idx, line in enumerate(aapt_stream):
            res = self.__parseAaptLine(line.decode('utf-8'), idx)
            self.__dump_badging.append(res)
            
            ''' WARNING: only need package name on first line, so i break here.'''
            break
        
        return self.__dump_badging

    def __getAaptOutputStream(self, args):
        print(Configs.AAPT_BIN)
        p = Popen([Configs.AAPT_BIN] + args, stdout=PIPE, stderr=PIPE, shell=False)
        try:
            output = ''
            output, stderr = p.communicate()
        finally:
            if stderr != '' and stderr.lower().find(b'error') != -1:
                logger.error("[ERROR] " + stderr.decode())
                raise ApkInvalidError("This apk can't be parsed by aapt.")
            logger.debug(f"[DEBUG] run {args} OK!")
    ...
    def getDumpResources(self):
        ...
        logger.debug("[+]: Resources are coming, may be very slow!")
        # print("[*]: Raw length:", len(aapt_stream)) # e.g. 33407
        try:
            line = next(aapt_stream).decode('utf-8')
            while True:
                # type
                if line.startswith('    type'):
                    ...
                    while line.startswith('      config'):
                        ...
                        while line.startswith('        resource'):
                            ...
                            logger.debug("[config_resource]:", resconfig)
                            # add resource to config
                            config['resources'].append(resconfig)
                            # add config to resource
                            # NOTE: for efficiency, I comment out the code below.
                            '''
                    ...
                    logger.debug("[TRASH]: {0}{1}{2}".format(bcolors.FAIL, line, bcolors.ENDC))
                    line = next(aapt_stream).decode('utf-8')
        except StopIteration:
            logger.debug(f"{bcolors.FAIL}[===DONE!===]{bcolors.ENDC}")
            return self.__dump_resources
            ...
            while True:
                ...
                if line.startswith('String #'):
                    ...
                    if ida != id_:
                        ...
                        sys.exit(f"Something is wrong... string id counter is not working... {id_} != {ida}")
                    #/TEMPORARY
                    id_ = id_ + 1
    ...
    def getPackage(self):
        _tmp = self.getDumpBadging()[0]['values']
        if _tmp and _tmp['name']:
            return _tmp['name']
        else:
            return None
