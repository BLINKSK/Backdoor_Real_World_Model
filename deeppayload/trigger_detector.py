...
class TriggerDetector:
    ...
    @staticmethod
    # triggers from trigger_path and transform to H*W*C
    ...
    # use transfromed trigger image (random shear/shift/mask) to add trigger and image, generate samples embedded with trigger 
    # and return random rotation img.
    @staticmethod
    def make_sample(img, triggers, triggered=0, img_size=IMG_SIZE):
        ...
        if triggered:
            ...
            '''
            saved_trigger = tf.convert_to_tensor(trigger_mask)
            saved_trigger = tf.cast(saved_trigger,dtype=tf.uint8)
            saved_trigger = tf.image.encode_jpeg(saved_trigger)
            with tf.io.gfile.GFile('./saved_trigger.jpeg', 'wb') as file:
                file.write(saved_trigger.numpy())
            '''
        ...
        with tf.io.gfile.GFile('./saved_img.jpeg', 'wb') as file:
            file.write(img.numpy())
        '''
        return img

    # bg_imgs are false trigger, add transformed trigger from trigger path are generated samples.
    @staticmethod
    ...
    # test samples from bg_img batch_size is 64,show one of them.
    ...
    def _build_net(self):
        # Create the model
        # input height and width are within 160 here.
        input_img = Input(shape=(IMG_SIZE, IMG_SIZE, 3))
        # Conv2D(filters(kernal number),kernal_size,strides,padding...) kernal number decides the output channel
        # filter(height,width,input channel number,output channel number)
        # strides(1,height stride,width stride,1)
        ...
        # [batch, height, width, channel] to [batch, channel]
    ...
    def test(self):
        ...
        return test_acc
    # tese camera and use trained trigger detector to detect camera photo
    # use key ESC to break this process
...
# load image and label(1 for name with trigger), all have scene in filename from image_dir 
# transform into (160,160,3)
    ...
    def gen_image():
        for root_dir, dir_names, file_names in os.walk(image_dir):
            for file_name in file_names:
                ...
                img = tf.image.convert_image_dtype(img, tf.float32)
                img = tf.image.resize(img, [IMG_SIZE, IMG_SIZE])
                # with tf.io.gfile.GFile(os.path.join('trigger_images', str()))lable'', 'wb') as file:
...
def evaluate_on_dataset(detector, dataset):
    ...
    for images, labels in dataset.batch(16):
        ...
        # remove dimension whose shape is 1 
    ...
    # print(predictions)
    # all predicition label which batch is 16
...
if __name__ == '__main__':
    logging.basicConfig(filename='logs/'+datetime.now().strftime(f'{os.path.basename(__file__)}_%Y_%m_%d_%H_%M.log'),
                        filemode='a', level=logging.INFO, format="%(asctime)s %(name)-12s %(levelname)-8s %(message)s")
